	...
	<div class="container my-5 border shadow p-5 w-75">
		...
		<form id="idFormAioCheckOut" method="post" action="ECPayServer" class="border shadow p-3 mb-5">			
			<div class="row text-primary ">				
				<div class="mb-3 col-sm-3">
					...
					<input type="text" name="TotalAmount" value="1050" class="form-control" />
					請帶整數，不可有小數點。僅限新台幣。
				</div>			
				...
				<div class="mb-3 col-sm-5">
					...
					<input type="text"	name="ItemName" value="馬桶刷850元x1#消毒水200元" class="form-control" />
					商品名稱<br/> 
		...
		<div class="row">
			<div class="mb-3 col-sm-12">
				...
				<hr/>
				付款結果通知我們伺服端的方式(可二選一)：<br/>
				8. ReturnURL：付款完成通知回傳網址。當消費者付款完成後，綠界會將付款結果參數以幕後(Server POST)回傳到該網址(該網址須是一個固定IP且使用https協定)。<br/>				  			
				9. OrderResultURL：付款完成通知回傳網址。當消費者付款完成後，綠界會將付款結果參數以幕前(Client POST)回傳到該網址。<br/>
				   (上列二項直接設定於伺服端程式)
				<hr/>
				(若9與X同時設定，將會以9為主。)<br/>
				X. ClientBackURL：設定此選項會使綠界付款成功畫面增加「返回商店」按鈕，消費者點選此按鈕後，會將頁面導回到此設定的網址。(注意：導回時不會帶付款結果到此網址，只是將頁面(以GET方法)導回而已。)<br/>
				  (上列直接設定於伺服端程式)<br/>
				<hr/>
